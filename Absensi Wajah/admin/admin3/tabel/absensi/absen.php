...
echo $jam_masuk = baca_database("","jam_masuk","select * from data_jadwal where hari='$hari'");;
echo "<br>";
echo $jam_mulai = menjumlahkan_waktu($jam_masuk,'+0 day +0 hour -15 minutes +0 seconds');
echo " s/d ";
echo $jam_limit = menjumlahkan_waktu($jam_masuk,'+0 day +0 hour +15 minutes +0 seconds');
echo "<br></font>";
...
$jam_limit_pindah = menjumlahkan_waktu($jam_masuk,'+0 day +3 hour +15 minutes +0 seconds');

if (strtotime($jam_sekarang) > strtotime($jam_limit))
{
	if (strtotime($jam_sekarang) > strtotime($jam_limit_pindah))
	{
	echo  "<br><font color='#6C70B1'>Waktu Absensi Berakhir";
	}
	else
	{
		echo "<br><font color='red'>Terlambat Absensi - Waktu Terlambat : ";
...
<table <?php tabel(100,'%',1,'left');  ?> >
			<tr>								  
			   ...
			   <th align="center" ><center><h4>nik</h4></center></th>
			...
			<?php
               ...
               <tr class="event2">	
				 ...
				 $tanggal= (date('Y-m-d'));
				 $ada =  cek_database("","","","select * from data_absensi where id_pegawai='$id_pegawai' and tanggal='$tanggal' and jenis_absensi='masuk'");
				 $id_absensi =  baca_database("","id_absensi","select * from data_absensi where id_pegawai='$id_pegawai' and id_jadwal='proses'");
				 ...
				 if ($ada=="ada")
				{
					...
					<font color="red"><b>
					<?php
					 echo ucwords(baca_database("","status","select * from data_absensi where id_pegawai='$id_pegawai' and id_jadwal='proses'"));
					?>
					</b></font>
					</center>
					<?php
				...
				else
				{
				 ...
				 Absen Manual :
				 <a onclick="window.open('sakit.php?id_jadwal=<?php echo $id_jadwal;?>&id_pegawai=<?php echo $id_pegawai;?>');"  class="btn btn-info btn-xs">Sakit</a>
				 <a onclick="window.open('izin.php?id_jadwal=<?php echo $id_jadwal;?>&id_pegawai=<?php echo $id_pegawai;?>');"  class="btn btn-warning btn-xs">Izin</a>
				 <a onclick="window.open('alfa.php?id_jadwal=<?php echo $id_jadwal;?>&id_pegawai=<?php echo $id_pegawai;?>');" class="btn btn-danger btn-xs">Alfa</a>
				 <a onclick="window.open('hadir.php?id_jadwal=<?php echo $id_jadwal;?>&id_pegawai=<?php echo $id_pegawai;?>');" class="btn btn-success btn-xs">Hadir</a>
				 </center>
				<?php } ?>
